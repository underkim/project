...
export default function HealthPage() {
  ...
  const [editingEx, setEditingEx] = useState<{ id: number; exercise_type: string; duration_minutes: string; note: string } | null>(null);
  const [editingSl, setEditingSl] = useState<{ id: number; sleep_hours: string; quality: string; note: string } | null>(null);
  ...
  const sleepByDow = sleeps.slice(0, 30).reduce<Record<number, { sum: number; count: number }>>((acc, sl) => {
  ...
  const scatterMinHours = hasSleepScatter ? Math.max(0, Math.min(...sleepScatter.map(s => s.sleep_hours)) - 0.5) : 0;
  const scatterMaxHours = hasSleepScatter ? Math.max(...sleepScatter.map(s => s.sleep_hours)) + 0.5 : 10;
  ...
  const topTypes = Object.entries(exByType).sort(([, a], [, b]) => b - a).slice(0, 5);
  ...
  const filteredExercises = exMonthFilter === 'all' ? exercises : exercises.filter(ex => ex.log_date.startsWith(exMonthFilter));
  const slMonths = [...new Set(sleeps.map(sl => sl.log_date.slice(0, 7)))].sort().reverse();
  const filteredSleeps = slMonthFilter === 'all' ? sleeps : sleeps.filter(sl => sl.log_date.startsWith(slMonthFilter));

  const qualityLabels = ['', '최악', '나쁨', '보통', '좋음', '최고'];
  const qualityColors = ['', 'bg-red-100 text-red-600', 'bg-orange-100 text-orange-600', 'bg-slate-100 text-slate-600', 'bg-emerald-100 text-emerald-700', 'bg-emerald-200 text-emerald-800'];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="flex items-center justify-between mb-1.5">
            ...
            {editingExGoal ? (
              <div className="flex items-center gap-1">
                <input type="number" min="1" max="7" value={exGoalInput}
                  onChange={e => setExGoalInput(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter') saveExGoal(); if (e.key === 'Escape') setEditingExGoal(false); }}
                  ...
                  autoFocus placeholder="일" />
                <button onClick={saveExGoal} className="text-xs text-blue-600 font-medium">확인</button>
              </div>
            ) : (
              <button onClick={() => { setExGoalInput(String(exWeekGoal || '')); setEditingExGoal(true); }}
                className="text-slate-300 hover:text-slate-500 transition-colors" title="주간 목표 설정">
                <Target size={12} />
              </button>
            )}
          </div>
            ...
            <div className="mt-2">
              <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-emerald-400 rounded-full transition-all"
                  style={{ width: `${Math.min(100, Math.round(((summary?.exercise_days_this_week ?? 0) / exWeekGoal) * 100))}%` }} />
              </div>
        ...
        <div className="border border-slate-100 rounded-xl px-4 py-4">
          <div className="flex items-center justify-between mb-1.5">
            ...
            {editingSlGoal ? (
              <div className="flex items-center gap-1">
                <input type="number" min="1" max="12" step="0.5" value={slGoalInput}
                  onChange={e => setSlGoalInput(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter') saveSlGoal(); if (e.key === 'Escape') setEditingSlGoal(false); }}
                  ...
                  autoFocus placeholder="h" />
                <button onClick={saveSlGoal} className="text-xs text-blue-600 font-medium">확인</button>
              </div>
            ) : (
              <button onClick={() => { setSlGoalInput(String(slNightlyGoal || '')); setEditingSlGoal(true); }}
                className="text-slate-300 hover:text-slate-500 transition-colors" title="목표 수면 시간 설정">
                <Target size={12} />
              </button>
            )}
          </div>
            ...
            <div className="mt-2">
              <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-indigo-400 rounded-full transition-all"
                  style={{ width: `${Math.min(100, Math.round((summary.avg_sleep_hours_this_week / slNightlyGoal) * 100))}%` }} />
              </div>
      ...
      {sleep30ChartData.length >= 7 && (
        <div className="border border-slate-100 rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">수면 추이 (시간)</p>
            {avgSleepHours != null && (
          ...
          <ResponsiveContainer width="100%" height={150}>
            <LineChart data={sleep30ChartData} margin={{ top: 4, right: 8, left: -24, bottom: 0 }}>
              ...
              <Tooltip formatter={(v, name) => name === '수면' ? [`${v}시간`, '수면'] : [`${v}/5`, '품질']} contentStyle={{ fontSize: 12 }} />
              {avgSleepHours != null && (
                <ReferenceLine y={avgSleepHours} stroke="#94a3b8" strokeDasharray="4 4" />
              )}
              <Line type="monotone" dataKey="수면" stroke="#0f172a" strokeWidth={1.5}
                dot={{ r: 2, fill: '#0f172a' }} activeDot={{ r: 4 }} connectNulls={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {hasDowData && sleeps.length >= 7 && (
        <div className="border border-slate-100 rounded-xl px-5 py-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">요일별 평균 수면 (최근 30일)</p>
            {avgSleepHours != null && (
          ...
          <div className="flex items-end gap-2 justify-between">
            {sleepDowData.map(({ day, avg, isWeekend }) => {
              const pct = avg != null && avgSleepHours ? Math.min(100, Math.round((avg / Math.max(avgSleepHours * 1.3, 10)) * 100)) : 0;
              ...
              return (
                <div key={day} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-[10px] text-slate-400">{avg != null ? `${avg}h` : '—'}</span>
                  <div className="w-full flex flex-col justify-end" style={{ height: 60 }}>
                    <div
                      className={`w-full rounded-t-sm transition-all ${
                        avg == null ? 'bg-slate-50' :
                        isHigh ? 'bg-indigo-400' :
                        isLow ? 'bg-rose-300' :
                        isWeekend ? 'bg-slate-400' : 'bg-slate-300'
                      }`}
                      style={{ height: avg != null ? `${pct}%` : '4px' }}
                    />
                  </div>
                  <span className={`text-[11px] font-medium ${isWeekend ? 'text-slate-500' : 'text-slate-400'}`}>{day}</span>
                </div>
              );
            })}
          </div>
          <p className="text-[10px] text-slate-300 mt-2">
            <span className="inline-block w-2 h-2 rounded-sm bg-rose-300 mr-1 align-middle" />평균보다 낮음
            <span className="inline-block w-2 h-2 rounded-sm bg-indigo-400 mx-1 ml-3 align-middle" />평균보다 높음
          </p>
        </div>
      )}
      ...
      {hasSleepScatter && (
        <div className="border border-slate-100 rounded-xl px-5 py-4">
          <p className="text-xs font-medium text-slate-500 mb-3 uppercase tracking-wide">수면 시간 vs 품질 (최근 30일)</p>
          <div className="relative" style={{ height: 120 }}>
            {/* Y축 라벨 (품질 1-5) */}
            <div className="absolute left-0 top-0 bottom-0 flex flex-col justify-between text-[9px] text-slate-300 pr-1" style={{ width: 20 }}>
              {[5, 4, 3, 2, 1].map(q => <span key={q}>{q}</span>)}
            </div>
            {/* 플롯 영역 */}
            <div className="absolute inset-0 ml-5">
              {/* 수평 격자선 */}
              {[1, 2, 3, 4, 5].map(q => (
                <div key={q} className="absolute w-full border-t border-slate-50"
              ...
              {sleepScatter.map(sl => {
                ...
                const qColors = ['', 'bg-red-400', 'bg-orange-400', 'bg-slate-400', 'bg-emerald-400', 'bg-emerald-600'];
                return (
                  <div
                    key={sl.id}
      ...
      {hasSleepHist && (
        <div className="border border-slate-100 rounded-xl px-5 py-4">
          <p className="text-xs font-medium text-slate-500 mb-3 uppercase tracking-wide">수면 시간 분포 ({sleeps.length}회)</p>
          <div className="flex items-end gap-2 justify-between">
                ...
                <div key={label} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <div className="w-full flex flex-col justify-end" style={{ height: 60 }}>
                    <div
                      className={`w-full rounded-t-sm transition-all ${
                        count === 0 ? 'bg-slate-50' :
                        isIdeal ? 'bg-indigo-400' : 'bg-slate-200'
                      }`}
          ...
          <p className="text-[10px] text-slate-400 mt-2 text-center">
            권장(7~9h) <span className="inline-block w-2 h-2 rounded-sm bg-indigo-400 align-middle mx-0.5" /> · 기타 <span className="inline-block w-2 h-2 rounded-sm bg-slate-200 align-middle mx-0.5" />
          </p>
        </div>
      )}
      ...
      {hasExDowData && exercises.length >= 5 && (
        <div className="border border-slate-100 rounded-xl px-5 py-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">요일별 누적 운동 (분)</p>
            <span className="text-xs text-slate-400">최다: {exDowData.reduce((a, b) => a.mins > b.mins ? a : b).day}요일</span>
          </div>
          <div className="flex items-end gap-2 justify-between">
                  ...
                  <div className="w-full flex flex-col justify-end" style={{ height: 60 }}>
                    <div
                      className={`w-full rounded-t-sm transition-all ${
                        ...
                        isWeekend ? 'bg-emerald-300' : 'bg-emerald-200'
                      }`}
      ...
      {hasExHeatmap && (
        <div className="border border-slate-100 rounded-xl px-5 py-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">운동 활동 (최근 16주)</p>
      ...
      {topTypes.length > 0 && (
        <div className="border border-slate-100 rounded-xl px-5 py-4">
          <p className="text-xs font-medium text-slate-500 mb-3 uppercase tracking-wide">운동 종류별 누적</p>
      ...
      {(exMonthlyData.length >= 2 || sleepMonthlyData.length >= 2) && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {exMonthlyData.length >= 2 && (
            <div className="border border-slate-100 rounded-xl p-5">
              <p className="text-xs font-medium text-slate-500 mb-4 uppercase tracking-wide">월별 운동 일수</p>
          ...
          {sleepMonthlyData.length >= 2 && (
            <div className="border border-slate-100 rounded-xl p-5">
              <p className="text-xs font-medium text-slate-500 mb-4 uppercase tracking-wide">월별 평균 수면 (시간)</p>
              <ResponsiveContainer width="100%" height={130}>
                <LineChart data={sleepMonthlyData} margin={{ top: 4, right: 8, left: -24, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f8fafc" />
                  <XAxis dataKey="month" tick={{ fontSize: 10, fill: '#94a3b8' }} />
                  <YAxis domain={['auto', 'auto']} tick={{ fontSize: 10, fill: '#94a3b8' }} />
                  <Tooltip formatter={(v) => [`${v}h`, '평균 수면']} contentStyle={{ fontSize: 12 }} />
                  {avgSleepHours && (
                    <ReferenceLine y={avgSleepHours} stroke="#cbd5e1" strokeDasharray="4 2"
                      label={{ value: `평균 ${avgSleepHours}h`, position: 'insideTopRight', fontSize: 9, fill: '#94a3b8' }} />
                  )}
                  <Line type="monotone" dataKey="평균" stroke="#6366f1" strokeWidth={2}
                    dot={{ r: 3, fill: '#6366f1' }} activeDot={{ r: 4 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
        ...
        {exMonths.length > 1 && (
          <div className="flex items-center gap-1.5 px-5 py-2.5 border-b border-slate-50 overflow-x-auto">
            {(['all', ...exMonths] as const).map(m => (
              <button key={m} onClick={() => setExMonthFilter(m)}
          ...
          <form onSubmit={submitExercise} className="px-5 py-4 bg-slate-50 border-b border-slate-100">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="sm:col-span-3">
                ...
                <input type="text" value={exForm.note} placeholder="오늘 운동 내용 또는 느낌..."
            ...
            editingEx?.id === ex.id ? (
              <div key={ex.id} className="px-5 py-3 bg-blue-50 border-l-2 border-blue-400">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-2">
                  <input type="text" value={editingEx.exercise_type}
                    onChange={e => setEditingEx({ ...editingEx, exercise_type: e.target.value })}
                    placeholder="운동 종류" className="text-sm border border-slate-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-400" />
                  <input type="number" value={editingEx.duration_minutes} min="1"
                    onChange={e => setEditingEx({ ...editingEx, duration_minutes: e.target.value })}
                  ...
                  <input type="text" value={editingEx.note}
              ...
              <div key={ex.id} className="flex items-start px-5 py-3 gap-4 hover:bg-slate-50 transition-colors group cursor-pointer"
                onClick={() => { if (deletingEx !== ex.id) setEditingEx({ id: ex.id, exercise_type: ex.exercise_type, duration_minutes: String(ex.duration_minutes), note: ex.note ?? '' }); }}>
                <span className="text-slate-400 text-xs w-24 shrink-0 mt-0.5">{ex.log_date}</span>
                <div className="flex-1 min-w-0">
                  <span className="font-medium text-sm text-slate-700 group-hover:text-blue-600 transition-colors">{ex.exercise_type}</span>
                  {ex.note && <p className="text-[11px] text-slate-400 mt-0.5 truncate">{ex.note}</p>}
                </div>
        ...
        {showSl && (
          <form onSubmit={submitSleep} className="px-5 py-4 bg-slate-50 border-b border-slate-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="sm:col-span-3">
                <label className="text-xs text-slate-500 mb-1 block">메모</label>
                <input type="text" value={slForm.note} placeholder="수면 특이사항..."
        ...
        {slMonths.length > 1 && (
          <div className="px-5 py-2.5 border-b border-slate-50 flex gap-1.5 overflow-x-auto bg-slate-50/50">
            {(['all', ...slMonths] as const).map(m => (
              <button key={m} onClick={() => setSlMonthFilter(m)}
            ...
            editingSl?.id === sl.id ? (
              <div key={sl.id} className="px-5 py-3 bg-blue-50 border-l-2 border-blue-400">
                <div className="grid grid-cols-3 gap-2 mb-2">
                  <input type="number" step="0.5" min="0.5" max="24" value={editingSl.sleep_hours}
                    onChange={e => setEditingSl({ ...editingSl, sleep_hours: e.target.value })}
                    placeholder="시간" className="text-sm border border-slate-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-400" />
                  <select value={editingSl.quality}
                    ...
                    {[5, 4, 3, 2, 1].map(q => <option key={q} value={q}>{qualityLabels[q]}</option>)}
                  </select>
                  <input type="text" value={editingSl.note}
              ...
              <div key={sl.id} className="flex items-start px-5 py-3 gap-4 hover:bg-slate-50 transition-colors group cursor-pointer"
                onClick={() => { if (deletingSl !== sl.id) setEditingSl({ id: sl.id, sleep_hours: String(sl.sleep_hours), quality: String(sl.quality), note: sl.note ?? '' }); }}>
                <span className="text-slate-400 text-xs w-24 shrink-0 mt-0.5">{sl.log_date}</span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm text-slate-700 group-hover:text-blue-600 transition-colors">{sl.sleep_hours}시간</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${qualityColors[sl.quality]}`}>
