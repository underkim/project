...
export default function TrackersPage() {
  ...
  const loadTrackers = useCallback(async () => {
    try {
      const list = await trackersApi.list();
      setTrackers(list);
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <div className="py-20 text-center text-sm text-slate-400">불러오는 중...</div> : trackers.length === 0 && !showCreate ? (
        <div className="rounded-2xl border border-dashed border-slate-300 py-16 text-center"><ListChecks className="mx-auto text-slate-300" size={36}/><h2 className="mt-4 font-semibold text-slate-800">첫 추적 항목을 만들어보세요</h2><p className="mt-2 text-sm text-slate-500">운동 시간, 독서량, 기분, 습관처럼 무엇이든 기록할 수 있어요.</p><button onClick={()=>setShowCreate(true)} className="mt-5 rounded-xl bg-slate-900 px-4 py-2.5 text-sm text-white">추적 시작하기</button></div>
      ) : (
