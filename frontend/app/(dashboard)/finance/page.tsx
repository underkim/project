...
export default function FinancePage() {
  ...
  function startEdit(r: AssetRecordResponse) {
    ...
    setEditForm({
      ...
      total_assets: String(r.total_assets),
      monthly_income: String(r.monthly_income),
      monthly_expense: String(r.monthly_expense),
      note: r.note ?? '',
    });
  }
  ...
  function saveSrGoal() {
    ...
    if (v > 0 && v <= 100) {
      localStorage.setItem(SAVINGS_RATE_GOAL_KEY, String(v));
      setSavingsRateGoal(v);
    }
  ...
  const onTrack =
    goal?.required_monthly_saving != null && avgMonthlySavingsAmt != null
  ...
  const savingsDeclineAlert = (() => {
    ...
    return (
      (recent[1].savings_rate ?? 0) < (recent[0].savings_rate ?? 0) &&
        ...
        <div className="border border-slate-100 rounded-xl px-5 py-4">
          <div className="flex items-center justify-between mb-1.5">
            ...
            {!editingGoal ? (
              ...
              >
                <Target size={10} />
                {goal?.target_amount
                  ? `목표 ${goal.target_amount.toLocaleString()}만`
                  : '목표 설정'}
              </button>
            ) : (
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  min="0"
                  step="0.1"
                  value={goalForm.expected_annual_return_rate}
                  onChange={(e) =>
                    setGoalForm({ ...goalForm, expected_annual_return_rate: e.target.value })
                  }
                  title="예상 연 수익률 (%)"
                  placeholder="수익률%"
                ...
                <button
                  onClick={saveGoal}
                  disabled={savingGoal}
                  className="text-[10px] px-1.5 py-0.5 bg-slate-100 rounded hover:bg-slate-200 text-slate-600 disabled:opacity-50"
                >
          ...
          {assetDelta != null && (
            ...
            >
              ...
              <span>
                {assetDelta >= 0 ? '+' : ''}
                {assetDelta.toLocaleString()}만원 (전회 대비)
              </span>
            </div>
          )}
              ...
              {goal.achieved ? (
                ...
                goal.required_monthly_saving != null && (
                  <p className="text-[10px] text-slate-400 mt-1">
                    {goal.expected_annual_return_rate > 0
                      ? `연 ${goal.expected_annual_return_rate}% 수익률 반영 시 `
                      : ''}
                    매달{' '}
                    <span className="font-medium text-slate-600">
                      {goal.required_monthly_saving.toLocaleString()}만원
                    </span>{' '}
                    저축 필요
                    {onTrack != null && (
                      <span
                        className={
                          onTrack ? 'text-emerald-600 font-medium' : 'text-amber-600 font-medium'
                        }
                      >
                        {' '}
                        ·{' '}
                        {onTrack
                          ? '현재 페이스로 달성 가능'
                          : `현재 평균 ${avgMonthlySavingsAmt?.toLocaleString()}만원으로는 부족`}
                      </span>
                    )}
                  </p>
                )
        ...
        <div className="border border-slate-100 rounded-xl px-5 py-4">
          ...
          {summary.avg_savings_rate != null && (
            <p className="text-xs mt-1 text-slate-400">
              {summary.avg_savings_rate >= 30
                ? '우수한 저축률'
                : summary.avg_savings_rate >= 15
                  ? '양호한 저축률'
                  : '저축률을 높여보세요'}
            </p>
          )}
          {savingsRateGoal > 0 && summary.avg_savings_rate != null && (
            <div className="mt-2">
              <div className="flex justify-between text-[10px] text-slate-400 mb-1">
                ...
                <span
                  className={
                    summary.avg_savings_rate >= savingsRateGoal
              ...
              <div className="h-1 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${summary.avg_savings_rate >= savingsRateGoal ? 'bg-emerald-400' : 'bg-blue-400'}`}
                  style={{
                    width: `${Math.min(100, Math.round((summary.avg_savings_rate / savingsRateGoal) * 100))}%`,
                  }}
                />
              </div>
            </div>
              ...
              {editingSrGoal ? (
                <div className="flex items-center gap-1">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') saveSrGoal();
                      if (e.key === 'Escape') setEditingSrGoal(false);
                    }}
                    autoFocus
                    placeholder="%"
                    className="w-16 border border-slate-200 rounded px-1.5 py-0.5 text-xs focus:outline-none focus:ring-1 focus:ring-slate-900"
                  ...
                  <button
                    onClick={() => setEditingSrGoal(false)}
                    className="text-[10px] text-slate-400 hover:text-slate-600"
                  >
                    취소
                  </button>
          ...
          {savingsRateGoal > 0 &&
            ...
            (editingSrGoal ? null : (
              <button
                onClick={() => {
                  setSrGoalInput(String(savingsRateGoal));
                  setEditingSrGoal(true);
                }}
          ...
          {monthlyBudget > 0 && latest?.monthly_expense != null && (
            <div className="mt-2 pt-2 border-t border-slate-50">
              <div className="flex justify-between text-[10px] text-slate-400 mb-1">
                ...
                <span
                  className={
                    latest.monthly_expense > monthlyBudget
              ...
              <div className="h-1 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${latest.monthly_expense > monthlyBudget ? 'bg-red-400' : 'bg-emerald-400'}`}
                  style={{
                    width: `${Math.min(100, Math.round((latest.monthly_expense / monthlyBudget) * 100))}%`,
                  }}
                />
              </div>
            </div>
            ...
            {goal.scenarios.map((s) => (
              <div
                ...
                className={`text-center rounded-lg px-2 py-2.5 ${
                  s.annual_return_rate === goal.expected_annual_return_rate
                    ? 'bg-slate-900 text-white'
                    : 'bg-slate-50 text-slate-700'
                }`}
              >
                <p
                  className={`text-[10px] ${s.annual_return_rate === goal.expected_annual_return_rate ? 'text-slate-300' : 'text-slate-400'}`}
                >
                  연 {s.annual_return_rate}%
                </p>
                <p className="text-sm font-semibold mt-0.5">
                  {s.required_monthly_saving.toLocaleString()}만
                </p>
              </div>
            ))}
          </div>
          <p className="text-[10px] text-slate-400 mt-3">
            저축만 할 때(연 0%)와 투자할 때 필요한 월 저축액을 비교합니다. 수익률이 높을수록 같은
            목표에 필요한 저축액이 줄어듭니다.
          </p>
        </div>
      )}
        ...
        <div className="border border-slate-100 rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
              ...
              />
              <Tooltip
                formatter={(v) => [`${(v as number).toLocaleString()}만원`, '자산']}
        ...
        <div className="border border-slate-100 rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={incomeExpenseData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              />
              <Tooltip
                formatter={(v, name) => [`${(v as number).toLocaleString()}만원`, name as string]}
                contentStyle={{ fontSize: 12 }}
              />
            ...
            <table className="w-full text-sm min-w-[560px]">
              <thead>
                <tr className="border-b border-slate-100">
                  {['날짜', '총 자산', '수입', '지출', '저축액', '저축률', '메모', ''].map((h) => (
                    <th key={h} className="text-left px-4 py-3 text-xs font-medium text-slate-400">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredForTable.map((r, idx) => {
                  ...
                  if (editingId === r.id) {
                    const inCls =
                      'w-full border border-slate-200 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400';
                    return (
                      <tr key={r.id} className="bg-blue-50">
                        <td className="px-2 py-2">
                          <input
                            ...
                            onChange={(e) =>
                              setEditForm({ ...editForm, record_date: e.target.value })
                            }
                            className={inCls}
                          />
                        </td>
                        <td className="px-2 py-2">
                          <input
                            ...
                            onChange={(e) =>
                              setEditForm({ ...editForm, total_assets: e.target.value })
                            }
                            className={inCls}
                          />
                        </td>
                        <td className="px-2 py-2">
                          <input
                            ...
                            onChange={(e) =>
                              setEditForm({ ...editForm, monthly_income: e.target.value })
                            }
                            className={inCls}
                          />
                        </td>
                        <td className="px-2 py-2">
                          <input
                            type="number"
                            min="0"
                            value={editForm.monthly_expense}
                            onChange={(e) =>
                              setEditForm({ ...editForm, monthly_expense: e.target.value })
                            }
                            className={inCls}
                          />
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1 justify-end">
                            <button
                              ...
                              disabled={mutating.has(`update_${r.id}`)}
                              className="flex items-center gap-1 text-[10px] px-2 py-0.5 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              {mutating.has(`update_${r.id}`) && (
                            ...
                            <button
                              ...
                              disabled={mutating.has(`update_${r.id}`)}
                              className="text-[10px] px-2 py-0.5 text-slate-400 hover:text-slate-600 disabled:opacity-50"
                            >
                              취소
                            </button>
                          </div>
                  ...
                  return (
                    <tr
                      ...
                      onClick={() => {
                        if (deletingId !== r.id) startEdit(r);
                      }}
                    >
                      ...
                      <td className="px-4 py-3 font-medium text-slate-900 group-hover:text-blue-600 transition-colors">
                        ...
                        {delta != null && (
                          <div
                            className={`text-[10px] font-normal ${delta >= 0 ? 'text-emerald-500' : 'text-red-400'}`}
                          >
                            {delta >= 0 ? '+' : ''}
                      ...
                      <td className="px-4 py-3 text-slate-500">
                        {r.monthly_income.toLocaleString()}만
                      </td>
                      <td className="px-4 py-3 text-slate-500">
                        {r.monthly_expense.toLocaleString()}만
                      </td>
                      <td className="px-4 py-3 text-slate-700 font-medium">
                        {r.savings_amount.toLocaleString()}만
                      </td>
                      <td className="px-4 py-3">
                        {r.savings_rate != null ? (
                          <span
                            className={`text-xs font-medium ${r.savings_rate >= 20 ? 'text-emerald-600' : 'text-slate-500'}`}
                          >
                            {r.savings_rate}%
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3 text-slate-400 text-xs max-w-[80px] truncate">
                        {r.note ?? '—'}
                      </td>
                      <td className="px-4 py-3 text-right">
                        {deletingId === r.id ? (
                          <DeleteConfirm
                            ...
                            disabled={mutating.has(`delete_${r.id}`)}
                          />
                        ) : (
                          <button
                            onClick={(e) => {
