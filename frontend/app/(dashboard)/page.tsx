...
function readLocalGoals() {
  ...
  return {
    ...
    eng:
      parseInt(
        localStorage.getItem(`eng_goal_${now.getFullYear()}_${now.getMonth() + 1}`) ?? '0',
        10,
      ) || 0,
      ...
      {lines.map((line, i) => {
        if (/^### (.+)/.test(line)) {
          return (
            <p
              key={i}
              className="font-bold text-slate-900 text-xs uppercase tracking-wider mt-4 first:mt-0"
            >
              {line.replace(/^### /, '')}
            </p>
          );
        ...
        if (/^## (.+)/.test(line)) {
          return (
            <p key={i} className="font-bold text-slate-900 text-base mt-4 first:mt-0">
              {line.replace(/^## /, '')}
            </p>
          );
        }
        if (/^# (.+)/.test(line)) {
          return (
            <p key={i} className="font-bold text-slate-900 text-lg mt-4 first:mt-0">
              {line.replace(/^# /, '')}
            </p>
          );
        }
      ...
      <div className="bg-slate-900 text-white rounded-2xl px-5 sm:px-6 py-6 flex items-center gap-4 sm:gap-6">
        ...
        <Link href="/planner" className="relative shrink-0">
          <RingProgress
            ...
            size={96}
            stroke={8}
            color={currentPhase ? currentPhase.color : '#94a3b8'}
          />
          ...
          {[
            {
              ...
              value:
                finance?.latest_total_assets != null
                  ? `${finance.latest_total_assets.toLocaleString()}만`
                  : '—',
            ...
            },
            {
              ...
              label: '여행',
              value:
                travel?.ongoing && travel.ongoing > 0
                  ? '진행 중'
                  : `예정 ${travel?.upcoming ?? 0}개`,
          ...
          <div className="border border-slate-100 rounded-2xl px-5 py-3.5 hover:border-slate-200 hover:shadow-sm transition-all">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-emerald-500 font-medium bg-emerald-50 px-1.5 py-0.5 rounded-full">
                  현재 단계
                </span>
              </div>
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                전체 {planner?.total_items ?? 0}개 중 {planner?.completed_items ?? 0}개 완료
              </p>
            </div>
              ...
              {planner.phases.map((p) => {
                ...
                return (
                  <div key={p.name} className="flex items-center gap-2">
                    <span
                      className={`text-[10px] w-16 shrink-0 truncate ${p.is_current ? 'font-semibold text-slate-700' : 'text-slate-400'}`}
                    >
                      {p.label || p.name}
                    </span>
            ...
            {health?.avg_sleep_hours_this_week && (
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-medium text-slate-700">
                  {health.avg_sleep_hours_this_week}시간
                </span>
              </div>
            )}
            {sleepQuality > 0 && (
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span
                    className={`text-[10px] font-medium ${sleepQuality >= 4 ? 'text-emerald-600' : sleepQuality <= 2 ? 'text-red-400' : 'text-slate-400'}`}
                  >
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="flex items-end gap-2">
                ...
                {finance?.asset_change != null && finance.asset_change !== 0 && (
                  <span
                    className={`text-xs font-semibold mb-0.5 ${finance.asset_change > 0 ? 'text-emerald-600' : 'text-red-500'}`}
                  >
                    {finance.asset_change > 0 ? '+' : ''}
                    {finance.asset_change.toLocaleString()}만
                  </span>
                )}
              </div>
            </div>
            {finance?.goal_target_amount != null && finance.goal_progress_pct != null && (
              <div>
                ...
                <ProgressBar value={finance.goal_progress_pct} max={100} color="bg-blue-500" />
                <p className="text-[10px] text-slate-400 mt-0.5">
                  목표 {finance.goal_target_amount.toLocaleString()}만원
                </p>
              </div>
            )}
            {savingsRate > 0 && (
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <span className="text-xs font-medium text-slate-700">
                  {growth?.english_days_this_month ?? 0}일
                </span>
                {engGoal > 0 ? (
            ...
            <ProgressBar
              ...
              color={
                engGoal > 0 && (growth?.english_days_this_month ?? 0) >= engGoal
                  ? 'bg-emerald-500'
                  : 'bg-violet-500'
              }
            />
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <div className="text-xs text-slate-400 space-y-0.5">
                ...
                <p>
                  완료 {(travel?.total ?? 0) - (travel?.upcoming ?? 0) - (travel?.ongoing ?? 0)}개
                </p>
              </div>
            </div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {travel.next_trip_start_date &&
                    (() => {
                      ...
                      const cls =
                        diff === 0 ? 'text-red-500' : diff <= 7 ? 'text-amber-500' : 'text-sky-500';
                      return (
                        <span className={`text-[11px] font-bold shrink-0 ${cls}`}>{label}</span>
                      );
                    })()}
                </div>
                ...
                {(travel.next_trip_checklist_total ?? 0) > 0 && (
                  <div>
                    <div className="flex justify-between text-[10px] text-slate-400 mb-0.5">
                      <span>체크리스트</span>
                      <span>
                        {travel.next_trip_checklist_done} / {travel.next_trip_checklist_total}
                      </span>
                    </div>
                    <ProgressBar
