...
export default function GrowthPage() {
  ...
  const [bookStatusFilter, setBookStatusFilter] = useState<
    'all' | 'reading' | 'completed' | 'wishlist'
  >('all');
  ...
  const [editingEng, setEditingEng] = useState<{
    id: number;
  ...
  async function load() {
    ...
    try {
      const [s, b, e] = await Promise.all([
        ...
        growthApi.listBooks(PAGE),
        growthApi.listEnglish(PAGE),
      ]);
      setSummary(s);
      setBooks(b);
      setBooksHasMore(b.length === PAGE);
      setEnglishLogs(e);
      setEngHasMore(e.length === PAGE);
    } catch {
      setLoadError(true);
    ...
    await withMutation('eng_create', async () => {
      try {
        await growthApi.createEnglish({
          log_date: engForm.log_date,
          activity_type: engForm.activity_type,
          duration_minutes: Number(engForm.duration_minutes),
          note: engForm.note || undefined,
        });
  ...
  function saveEngGoal() {
    ...
    if (v > 0) {
      localStorage.setItem(ENG_GOAL_KEY, String(v));
      setEngGoal(v);
      emitGoalChange();
    }
  ...
  const bookYears = [
    ...new Set(
      books
        .filter((b) => b.status === 'completed' && b.end_date)
        .map((b) => b.end_date!.slice(0, 4)),
    ),
  ]
    .sort()
    .reverse();
  ...
  const filteredBooks = books.filter((b) => {
    ...
    const matchYear =
      bookYearFilter === 'all' ||
      (b.status === 'completed' && (b.end_date ?? '').startsWith(bookYearFilter));
  ...
  const filteredEngLogs =
    engMonthFilter === 'all'
      ? englishLogs
      : englishLogs.filter((l) => l.log_date.startsWith(engMonthFilter));
  ...
  const bookPacePrediction = (() => {
    ...
    const currentMonthlyPace =
      completedThisYear > 0 ? Math.round((completedThisYear / monthsElapsed) * 10) / 10 : null;
    const projectedTotal =
      currentMonthlyPace != null
  ...
  const inputCls =
    'w-full border border-slate-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 bg-white';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="border border-slate-100 rounded-xl px-5 py-4">
          <div className="flex items-center justify-between mb-1.5">
            ...
            {!editingGoal ? (
              <button
                onClick={() => {
                  setGoalInput(bookGoal > 0 ? String(bookGoal) : '');
                  setEditingGoal(true);
                }}
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={saveGoal}
                  className="text-slate-500 hover:text-slate-900 text-[10px] px-1.5 py-0.5 bg-slate-100 rounded"
                >
                  확인
                </button>
              </div>
          ...
          <p className="text-2xl font-semibold text-slate-900">
            {summary?.books_completed_this_year ?? 0}권
          </p>
          {bookGoal > 0 && (
            <div className="mt-2">
              <div className="flex justify-between text-[10px] text-slate-400 mb-1">
                <span>
                  {summary?.books_completed_this_year ?? 0} / {bookGoal}권
                </span>
                <span>
                  {Math.min(
                    100,
                    Math.round(((summary?.books_completed_this_year ?? 0) / bookGoal) * 100),
                  )}
                  %
                </span>
              </div>
              <div className="h-1 bg-slate-100 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, Math.round(((summary?.books_completed_this_year ?? 0) / bookGoal) * 100))}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div className="mt-2 text-[10px] text-slate-400 space-y-0.5">
              <p>
                ...
                <span className="font-semibold text-violet-600">
                  {bookPacePrediction.neededPerMonth}권
                </span>{' '}
                필요
              </p>
              {bookPacePrediction.currentMonthlyPace != null &&
                bookPacePrediction.projectedTotal != null && (
                  <p>
                    현재 페이스({bookPacePrediction.currentMonthlyPace}권/월) 유지 시{' '}
                    <span
                      className={`font-semibold ${bookPacePrediction.projectedTotal >= bookGoal ? 'text-emerald-600' : 'text-amber-500'}`}
                    >
        ...
        <div className="border border-slate-100 rounded-xl px-5 py-4">
            ...
            {editingEngGoal ? (
              <div className="flex items-center gap-1">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') saveEngGoal();
                    if (e.key === 'Escape') setEditingEngGoal(false);
                  }}
              ...
              <button
                onClick={() => {
                  setEngGoalInput(String(engGoal || ''));
                  setEditingEngGoal(true);
                }}
          ...
          <p className="text-2xl font-semibold text-slate-900">
            {summary?.english_days_this_month ?? 0}일
          </p>
          <p className="text-xs text-slate-400 mt-1">
            {engHours > 0 ? `${engHours}시간 ` : ''}
            {engMins > 0 || engHours === 0 ? `${engMins}분` : ''}
          </p>
          {engStreak.current > 0 && (
            <p className="text-[10px] text-blue-500 font-medium mt-1">
              🔥 {engStreak.current}일 연속
            </p>
          )}
            ...
            <div className="mt-2">
              <div className="flex justify-between text-[10px] text-slate-400 mb-0.5">
                ...
                <span>
                  {Math.min(
                    100,
                    Math.round(((summary?.english_days_this_month ?? 0) / engGoal) * 100),
                  )}
                  %
                </span>
              </div>
              <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, Math.round(((summary?.english_days_this_month ?? 0) / engGoal) * 100))}%`,
                  }}
                />
              </div>
            </div>
      ...
      <div className="border border-slate-100 rounded-xl overflow-hidden">
        ...
        <div className="px-5 py-2.5 border-b border-slate-50 flex flex-wrap items-center gap-2 bg-slate-50/50">
          <input
            ...
            className="flex-1 min-w-0 text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-slate-400 bg-white"
          />
          <div className="flex items-center gap-1 flex-wrap">
            {(['all', 'reading', 'completed', 'wishlist'] as const).map((s) => (
              <button
                key={s}
                onClick={() => {
                  setBookStatusFilter(s);
                  if (s !== 'completed') setBookYearFilter('all');
                }}
        ...
        {showBookForm && (
          <form onSubmit={submitBook} className="px-5 py-4 bg-slate-50 border-b border-slate-100">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setBookForm({ ...bookForm, status: e.target.value as BookStatus })
                  }
                  className={inputCls}
                >
              ...
              <div>
                ...
                <StarRating
                  ...
                  onChange={(v) => setBookForm({ ...bookForm, rating: v })}
                />
              </div>
              <div className="sm:col-span-2">
            ...
            <div className="flex justify-end gap-2 mt-3">
              ...
              <button
                type="submit"
                disabled={mutating.has('book_create')}
          ...
          {books.length === 0 ? (
            ...
            filteredBooks.map((book) =>
              editingBook?.id === book.id ? (
                <div key={book.id} className="px-5 py-3 bg-blue-50 border-l-2 border-blue-400">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-2">
                    <input
                      ...
                      placeholder="제목"
                      className="text-sm border border-slate-200 rounded px-2 py-1 col-span-2 focus:outline-none focus:ring-1 focus:ring-blue-400"
                    />
                    <input
                      type="text"
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {[1, 2, 3, 4, 5].map((s) => (
                        <button
                          key={s}
                          type="button"
                          onClick={() => setEditingBook({ ...editingBook, rating: String(s) })}
                          className={`text-lg leading-none ${parseInt(editingBook.rating) >= s ? 'text-amber-400' : 'text-slate-200'}`}
                        >
                          ★
                        </button>
                      ))}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={mutating.has('book_edit')}
                    ...
                    >
                      {mutating.has('book_edit') && <Loader2 size={11} className="animate-spin" />}
                      저장
                    </button>
                    <button
                ...
                <div
                  key={book.id}
                  className="flex items-start px-5 py-3 gap-3 hover:bg-slate-50 transition-colors group cursor-pointer"
                  onClick={() => {
                    if (deletingBook !== book.id)
                      setEditingBook({
                        id: book.id,
                        title: book.title,
                        author: book.author ?? '',
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    {book.note && (
                      <p className="text-[11px] text-slate-400 mt-0.5 truncate">{book.note}</p>
                    )}
                  </div>
                  <select
                    value={book.status}
                  ...
                  {deletingBook === book.id ? (
                    <DeleteConfirm
                      ...
                      disabled={mutating.has(`book_delete_${book.id}`)}
                    />
                  ) : (
                    <button
                      onClick={(e) => {
        ...
        {showEngForm && (
          <form onSubmit={submitEng} className="px-5 py-4 bg-slate-50 border-b border-slate-100">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input
                  type="number"
                  min="1"
                  value={engForm.duration_minutes}
            ...
            <div className="flex justify-end gap-2 mt-3">
              ...
              <button
                type="submit"
                disabled={mutating.has('eng_create')}
                className="flex items-center gap-1.5 bg-slate-900 text-white px-3 py-1.5 rounded-lg text-sm font-medium hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              editingEng?.id === log.id ? (
                <div key={log.id} className="px-5 py-3 bg-blue-50 border-l-2 border-blue-400">
                  <div className="grid grid-cols-3 gap-2 mb-2">
                    <select
                      value={editingEng.activity_type}
                      onChange={(e) =>
                        setEditingEng({ ...editingEng, activity_type: e.target.value })
                      }
                      className="text-sm border border-slate-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-400"
                    >
                    ...
                    <input
                      type="number"
                      min="1"
                      value={editingEng.duration_minutes}
                      onChange={(e) =>
                        setEditingEng({ ...editingEng, duration_minutes: e.target.value })
                      }
                      placeholder="분"
                      className="text-sm border border-slate-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-400"
                    />
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={saveEngEdit}
                      disabled={mutating.has('eng_edit')}
                      className="flex items-center gap-1 text-xs px-3 py-1 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {mutating.has('eng_edit') && <Loader2 size={11} className="animate-spin" />}
                      저장
                    </button>
                    <button
                ...
                >
                  ...
                  {deletingEng === log.id ? (
                    <DeleteConfirm
                      onConfirm={() => deleteEnglish(log.id)}
                      onCancel={() => setDeletingEng(null)}
                      disabled={mutating.has(`eng_delete_${log.id}`)}
                    />
                  ) : (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setDeletingEng(log.id);
                      }}
