...
export default function CareerPage() {
  ...
  const filteredForChart = chartYear === 'all' ? ratings : ratings.filter(r => r.log_date.startsWith(chartYear));
  const chartData = [...filteredForChart].reverse().map(r => ({ date: r.log_date.slice(5), rating: r.rating, rank: r.rank_name }));
  ...
  const growthStats = (() => {
    ...
    const days = Math.round((new Date(latest.log_date).getTime() - new Date(first.log_date).getTime()) / 86400000);
  ...
  // 최근 3회 연속 하락 감지
  const recentDecline = ratings.length >= 3 &&
      ...
      {latestRating && (
        <div className="grid grid-cols-2 gap-3">
          <div className="border border-slate-100 rounded-xl px-5 py-4">
            <div className="flex items-center justify-between mb-1.5">
              ...
              {editingCfGoal ? (
                <div className="flex items-center gap-1">
                  <input type="number" min="1" max="4000" value={cfGoalInput}
                    onChange={e => setCfGoalInput(e.target.value)}
                    onKeyDown={e => { if (e.key === 'Enter') saveRatingGoal(); if (e.key === 'Escape') setEditingCfGoal(false); }}
                    autoFocus placeholder="목표"
                  ...
                  <button onClick={saveRatingGoal} className="text-[10px] px-1.5 py-0.5 bg-slate-100 rounded hover:bg-slate-200 text-slate-600">확인</button>
                </div>
              ) : (
                <button onClick={() => { setCfGoalInput(cfGoal > 0 ? String(cfGoal) : ''); setEditingCfGoal(true); }}
            ...
            <p className="text-3xl font-semibold" style={{ color: ratingColor(latestRating.rating) }}>
              {latestRating.rating}
            </p>
            ...
            {cfGoal > 0 && (
              <div className="mt-2">
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">{latestRating.rating} → {cfGoal} ({cfGoal - latestRating.rating > 0 ? `+${cfGoal - latestRating.rating}` : '달성!'})</p>
              </div>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-3">
          {growthStats && (
            <div className="flex-1 border border-slate-100 rounded-xl px-4 py-3 min-w-0">
              ...
              <p className={`text-lg font-semibold ${growthStats.totalGrowth >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                {growthStats.totalGrowth >= 0 ? '+' : ''}{growthStats.totalGrowth}
              </p>
              <p className="text-[11px] text-slate-400 mt-0.5">
                {growthStats.days}일간 · 월평균 {growthStats.monthlyGrowth >= 0 ? '+' : ''}{growthStats.monthlyGrowth}
              </p>
            </div>
          )}
          {recentDecline && (
      ...
      {chartData.length >= 2 && (
        <div className="border border-slate-100 rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">레이팅 추이</p>
            ...
            return (
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                  ...
                  <YAxis domain={['dataMin - 100', 'dataMax + 200']} tick={{ fontSize: 11, fill: '#94a3b8' }} />
                  ...
                  {thresholds.map(t => (
                    <ReferenceLine key={t.y} y={t.y} stroke={t.color} strokeDasharray="4 2" strokeOpacity={0.5}
                      label={{ value: t.label, position: 'insideTopRight', fontSize: 9, fill: t.color, fillOpacity: 0.8 }} />
                  ))}
                  <Line
              ...
              <div>
                ...
                <input type="number" min="0" max="4000" value={ratingForm.rating}
                  onChange={e => {
                    ...
                    setRatingForm({ ...ratingForm, rating: v, rank_name: isNaN(n) ? ratingForm.rank_name : inferRank(n) });
                  }}
            ...
            return editingRating?.id === r.id ? (
              <div key={r.id} className="px-5 py-3 bg-blue-50 border-l-2 border-blue-400">
                <div className="grid grid-cols-3 gap-2 mb-2">
                  <input type="date" value={editingRating.log_date}
                    onChange={e => setEditingRating({ ...editingRating, log_date: e.target.value })}
                    className="text-sm border border-slate-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-400" />
                  <input type="number" min="0" max="4000" value={editingRating.rating}
                    onChange={e => {
                      const v = e.target.value;
                      const n = parseInt(v, 10);
                      setEditingRating({ ...editingRating, rating: v, rank_name: isNaN(n) ? editingRating.rank_name : inferRank(n) });
                    }}
                    className="text-sm border border-slate-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-400" />
                  <select value={editingRating.rank_name}
                    onChange={e => setEditingRating({ ...editingRating, rank_name: e.target.value })}
                    className="text-sm border border-slate-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-400">
                    {cfRanks.map(rank => <option key={rank} value={rank}>{rank}</option>)}
                  </select>
                </div>
              ...
              <div key={r.id} className="flex items-center px-5 py-3 gap-4 hover:bg-slate-50 transition-colors group cursor-pointer"
                onClick={() => { if (deletingId !== r.id) setEditingRating({ id: r.id, log_date: String(r.log_date), rating: String(r.rating), rank_name: r.rank_name }); }}>
                <span className="text-slate-400 text-xs w-24 shrink-0">{r.log_date}</span>
                <span className="font-bold text-slate-800 text-sm group-hover:text-blue-600 transition-colors" style={{ color: ratingColor(r.rating) }}>{r.rating}</span>
