...
**여행 계획 — 반드시 확인 후 저장 (중요)**
여행 계획은 상담형 흐름이다. 절대 사용자에게 정보를 되묻는 응답과 동시에 travel 기록을 저장하지 않는다.
- 날짜·목적지 등 필수 정보가 없거나, 추천/탐색/"계획 짜줘" 단계 → action: null (actions 없음), 부족한 정보를 질문하거나 제안만 한다.
- 한 응답에서 질문(되묻기)과 create/update 액션을 동시에 넣지 않는다. 질문하면 반드시 action: null.
- 사용자가 구체적 정보(여행명/목적지/시작일/종료일)를 주고 명확히 "추가/저장/만들어/등록해줘" 라고 했을 때만 travel_trip 생성.
- 일정·체크리스트도 사용자가 확정·승인한 뒤에만 저장.

예시 (탐색 단계 — 저장 금지):
→ {{"reply":"제주 여행 좋네요! 언제 출발·도착 예정이고, 가고 싶은 곳이 있을까요? 알려주시면 일정을 제안해 드릴게요.","action":null,"module":null,"data":null,"filter":null}}

예시 (사용자가 확정·저장 요청 — 그때만 actions 사용). trip_name은 방금 만든 여행 이름과 정확히 일치해야 함.
사용자: "제주 여행 8월 1일~3일로 추가하고 1·2일차 일정도 넣어줘"
→ {{"reply":"제주 여행이랑 1·2일차 일정 추가했어요!","actions":[
...
**다단계 대화로 계획 수립**
- 사용자가 방향만 잡는 단계: action: null로 조언/질문 제공 (절대 저장하지 않음)
- 세부 내용 확정 후 사용자가 명시적으로 "추가/저장/만들어/등록해줘" 하면 그때 저장
- 질문을 하는 응답은 반드시 action: null (질문 + 저장 동시 금지)
- 여행 계획 시: 여행 기간·목적지 먼저 확인(action: null) → 사용자 확정·요청 후 travel_trip 생성 → 일정 추가 순서
"""
...
def _reply_seeks_confirmation(reply: str) -> bool:
    """AI 응답이 사용자에게 정보를 되묻거나 확인을 구하는지 감지."""
...
async def _process_multi_actions(session: AsyncSession, reply: str, actions: list) -> dict:
    ...
    if _reply_seeks_confirmation(reply):
        actions = [
            a for a in actions
            if not (a.get("module") in _TRAVEL_MODULES and a.get("action") in ("create", "update"))
        ]
        if not actions:
            return {
                "reply": reply, "saved": False, "saved_count": 0,
                "module": None, "modules": None, "action": None, "suggestions": None,
            }
    saved_count = 0
